...
namespace E_Shop.Application.Services.RoleServices
{
    ...
    {                                                                                            
        ...
        public async Task UpdateUserRole(int userId, List<int>? selectedNewRoles)
        {
            ...
            foreach (var userRole in userRoles)
                if (!selectedNewRoles.Any(id => id == userRole.Id)) await DeleteUserRole(userRole.Id);
            foreach (var roleId in selectedNewRoles)
            {
